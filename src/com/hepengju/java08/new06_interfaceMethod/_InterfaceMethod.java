 ...
 *              加入默认方法后, 接口本身就可以提供实现了, 不必再使用适配器
 *              比如SpringMVC5的处理器拦截器 HandlerInterceptor 就添加了默认实现, 以便实现本接口的时候 @Overide 需要的方法就好
 *               
 *      2.为什么需要加入"静态方法"呢?
 *          Collection, Collections 工具类
 *          Path      , Paths 工具类
 *          既然第一点已经加入默认方法,那就再加入静态方法使得接口和工具类统一在一起
 *      
 *      3.java9中接口也可以加入私有方法
...
public class _InterfaceMethod {
    ...
    @Test public void testDefaultMethodAndStaticMethod() {
        MyInter.out("我是静态方法");                   // 我是静态方法
        
        MyInter mi = s -> s + "**";
        String src = "apply";
        System.out.println(mi.apply(src));           // apply**
        System.out.println(mi.applyTwoTime(src));    // apply****
    }
